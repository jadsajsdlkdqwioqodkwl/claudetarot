...
const ENCABEZADOS = ["Fecha (Lima)", "WhatsApp", "Contacto", "Quién", "Vendedor", "Tipo", "Mensaje", "Origen anuncio", "Título anuncio", "ctwa_clid", "Notas", "Asesora asignada"];
...
export async function exportarChatsASheets(env) {
  ...
  const { results: mensajes } = await env.CRM_DB.prepare(
    `SELECT m.id, m.created_at, m.direction, m.type, m.body, m.file_name, m.sent_by,
            ...
            c.ctwa_clid, c.ad_source_type, c.ad_headline, c.notes, conv.assigned_agent
     FROM messages m
